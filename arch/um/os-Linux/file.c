...
int os_send_data(const void *buf, int len, unsigned long long offset)
{
    
    char * server_filename = "/tmp/sockets";
    char * client_filename = "/tmp/socketc";
    
    int sockfd;
    // this will be sent to server
    static char offBytes[maxArraySize]; // HACK static - single-threaded;
    int bytesOffSent;
    ...
    strncpy(claddr.sun_path, client_filename, sizeof(claddr.sun_path));
    ...
    strncpy(svaddr.sun_path, server_filename, sizeof(claddr.sun_path));
    ...
    // convert off, len, buff into byte array
    memcpy(offBytes, &offset, 8);
    memcpy(offBytes+8, &len, 4);
    memcpy(offBytes+12, buf, len);
    
    //send it
    bytesOffSent = write(sockfd, offBytes, len+12); 
    //printk("Request sent, bytesOffSent: %i\n", bytesOffSent);
    
    // this will contain response
    static char bufResp[maxArraySize];
    int bytesResp = read(sockfd, bufResp, len+16);
    
    unsigned long long respCode;
    unsigned long long respLen;

//    printk("bytesResp %i\n",bytesResp); // received Nr of bytes
    
    if(bytesResp >= 16){
        //extract response code and buff from bytearray
        memcpy(&respCode, bufResp, 8);
        memcpy(&respLen, bufResp+8, 8);
        //printk("RESPCODE: %u RESPLEN: %i\n", respCode, respLen);
        if(respCode == 1){
            if(len == respLen && bytesResp >= len+16){
                // change current buff pointer to response buf.
                memcpy(buf, bufResp+16, len);
            }
            else{
                printk("Response with code 1 but bytearray not long enough.\n");
            }
        }
        else{
            //printk("Response code doesn't equal 1.\n");
        }
        
    }
    else{
        //printk("Response array length shorter than 16 bytes.\n");
    }
